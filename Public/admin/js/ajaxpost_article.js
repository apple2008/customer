...
function ajax_post_article() {
	...
	$.ajax({
		...
		success : function(msg){
          ...
          if (msg.status==1) {
          	alert("添加成功");
          	window.location.href="/Admin/index/mindManage";
...
function sel_article_list() {
	var cate_id=document.sel_article.cate_id.value;
...
//意向客户转换成成交客户
function change_article(id,self) {
    // body...
    $.ajax({
        type : 'post',
        url : '/Admin/Article/ajaxChangeUser',
        data : {'id' : id},
        dataType : 'json',
        success : function(msg) {
            // body...
