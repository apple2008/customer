...
<div class="wrap js-check-wrap">
    <ul class="nav nav-tabs">
		<li class="active"><a href="<?php echo U('/Admin/index/majorManage');?>" >行程管理</a></li>
		<li><a href="<?php echo U('/Admin/index/collegeMajorAdd');?>" >添加本周行程</a></li>
		<li><a href="<?php echo U('/Admin/index/collegeMajorNextAdd');?>" >添加下周行程</a></li>
	...
	<form name="sel_college" class="well form-search" action="<?php echo U('/Admin/index/majorManage');?>" method="post">
		...
		<select name="aid" style="width: 100px;" onclick="getSchool($(this).val())">
		   ...
		   <?php $__FOR_START_17705__=0;$__FOR_END_17705__=count($area);for($i=$__FOR_START_17705__;$i < $__FOR_END_17705__;$i+=1){ ?><option value="<?php echo ($area[$i]['id']); ?>"><?php echo ($area[$i]['name']); ?></option><?php } ?>
		...
		<select name="lid" id="level" style="width: 100px;">
			<option value="0">全部</option>
		   <?php $__FOR_START_8366__=0;$__FOR_END_8366__=count($level);for($i=$__FOR_START_8366__;$i < $__FOR_END_8366__;$i+=1){ ?><option value="<?php echo ($level[$i]['id']); ?>"><?php echo ($level[$i]['name']); ?></option><?php } ?>
	...
	<form name="college_list" class="js-ajax-form" novalidate="novalidate">
		...
		<table class="table table-hover table-bordered table-list">
			...
			<tbody>
			  <?php $__FOR_START_7759__=0;$__FOR_END_7759__=count($college_list);for($i=$__FOR_START_7759__;$i < $__FOR_END_7759__;$i+=1){ ?><tr>
