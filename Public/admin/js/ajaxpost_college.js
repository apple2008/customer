...
function editCollege() {
    ...
    $.ajax({
        ...
        success : function(msg) {
            if (msg.status==1) {
                alert('操作成功');
                window.location.href="/Admin/index/dealManage";
            }else{
                alert('操作失败');
            }
            console.log(msg);
...
function addCollege() {
    ...
    $.ajax({
        ...
        success : function(msg) {
            if (msg.status==1) {
                alert('操作成功');
                window.location.href="/Admin/index/majorManage";
            }else{
                alert('操作失败');
            }
            console.log(msg);
...
//删除数据
function del_college(id,self) {
	// body...
	$.ajax({
		type : 'post',
		url : '/Admin/College/ajaxDelSchool',
		data : {'id' : id},
		dataType : 'json',
		success : function(msg) {
			// body...
...
//首款转换成项目已完成
function change_college(id,self) {
    // body...
    $.ajax({
        type : 'post',
        url : '/Admin/College/ajaxChangeUser',
        data : {'id' : id},
        dataType : 'json',
        success : function(msg) {
            // body...
