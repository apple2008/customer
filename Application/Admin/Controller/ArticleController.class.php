...
class ArticleController extends Controller {
    ...
    {
        $return_addArticle_id=M("mind_user")->add($data);
    ...
    public function articlePart($article_id)
    {
        ...
        $return=M('mind_user')->where($data)->find();
    ...
    public function delArticle($id)
    {
        ...
        return M('mind_user')->where("id = $id")->delete();
    }
    //修改文章的单个值
    ...
    public function changeUser($id)
    {
        ...
        //获取意向客户信息写入成交客户表，删除原有的意向客户表
        $data['id'] = $id;
        $mind_user_info = M('mind_user')->where($data)->find();
        $deal_data['project'] = $mind_user_info['project'];
        $deal_data['corp_name'] = $mind_user_info['corp_name'];
        $deal_data['status'] = 1;
        $deal_data['address'] = $mind_user_info['address'];
        $deal_data['position'] = $mind_user_info['position'];
        $deal_data['name'] = $mind_user_info['name'];
        $deal_data['telphone'] = $mind_user_info['telphone'];
        $deal_data['mobile'] = $mind_user_info['mobile'];
        $deal_data['email'] = $mind_user_info['email'];
        $deal_data['business'] = $mind_user_info['business'];
        $deal_data['admin_id'] = $adminUserInfo['id'];
        $deal_data['create_time'] = date('Y-m-d H:i:s');
        $return_id = M("deal_user")->add($deal_data);
        if ($return_id){
            return M('mind_user')->where("id = $id")->delete();
        }
    }
    ...
    public function uploadArticle()
    {
        ...
        $article=I('post.post');
        $adminUserInfo = $_SESSION['AdminUserInfo'];
        $article['admin_id']= $adminUserInfo['id'];
       ...
       // foreach ($term as $key => $menu_id) {
            # code...
            $Article_id=$this->addArticle($article);
